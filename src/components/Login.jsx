...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const res = await axiosRequest.post("/auth", { email, password });
  ...
  return (
    <div className="min-h-screen absolute top-0 w-[100%] z-50 flex items-center justify-center bg-gradient-to-r from-gray-900 to-black text-white">
      <div className="relative w-full max-w-md bg-black/70 backdrop-blur-lg rounded-2xl shadow-xl p-8">
        ...
        <p className="text-sm text-gray-400 mt-4 relative z-10 text-center">
          Нет аккаунта?{" "}
          <Link href="/registration" className="text-[#0c7ff2] hover:underline">
            Зарегистрироваться
          </Link>
        </p>
		  <p className="text-sm text-gray-400 mt-4 relative z-10 text-center">
